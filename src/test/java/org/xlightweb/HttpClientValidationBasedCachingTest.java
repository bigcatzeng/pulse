...
*/
public final class HttpClientValidationBasedCachingTest  {
	...
	public void testETag() throws Exception {
        ...
        Assert.assertEquals(200, resp.getStatus());
        Assert.assertTrue(resp.getHeader(CacheHandler.XHEADER_NAME).startsWith("HIT - revalidated"));
        Assert.assertEquals("test", resp.getBody().readString());
    ...
    public void testLastModificationInvalid() throws Exception {
        ...
        IHttpRequestHandler reqHdl = new IHttpRequestHandler() {
            ...
            public void onRequest(IHttpExchange exchange) throws IOException, BadMessageException {
                ...
                IHttpRequest request = exchange.getRequest();
                String ifModifiedSince = request.getHeader("If-Modified-Since");
                if ((ifModifiedSince != null) && (ifModifiedSince.equals(new Date()))) {
                    exchange.send(new HttpResponse(304));
                    return;
                }
